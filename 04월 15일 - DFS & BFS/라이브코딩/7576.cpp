...
int bfs(int rows, int cols, int cnt, vector<vector<int>> &matrix, queue<ci> &que) {
    // 상, 하, 좌, 우
    int dr[] = {-1, +1, 0, 0};
    int dc[] = {0, 0, -1, +1};
...
int main() {
    ...
    int cnt = 0;    // 익지 않은 토마토의 수
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cin >> matrix[i][j];
            if (matrix[i][j] == 1) {    // 이미 익은 토마토의 좌표 큐에 삽입
                que.push({i, j});       // 탐색의 시작점이 된다.
