...
//다른 위치에서 시작해서 서로를 향해 가까워지는 투 포인터
//두 용액의 합 중 0에 제일 가까운 용액 리턴
ci findLiquid(int left, int right, vector<int> &liquid) {
    ...
    while (left < right) {
        ...
        if (abs(mix) < min_value) { //0에 가까운 값을 갱신할 수 있는지 확인
            min_value = abs(mix);
            ans = {liquid[left], liquid[right]};
        }
        if (mix < 0) { //0보다 작다면 더 큰 값을 만들어야 함 -> left 이동
            left++;
        } else { //0보다 크다면 더 작은 값을 만들어야 함 -> right 이동
            right--;
        }
    }
    return ans;
