 ...
 * 2. 출력에서 (ans == 3 * INF)을 하지 않고 INF 값 자체를 3배 해준 값으로 초기화 하는 이유
 *    조건에 의하면 E의 값이 0일 수도 있음. 이 경우에선 1->1->N->N의 값이 INF이라 경로가 없음에도 출력 시 조건이 (ans == 3 * INF)라면 조건에 걸리지 않음.
 *    따라서 경로가 없는 경우를 애초에 3배 해준 값으로 해야 함 => INF값 자체를 3배 해준 값으로 초기화하면 해결 가능
 */
